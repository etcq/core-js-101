...
function getSumBetweenNumbers(n1, n2) {
  return Array.from({ length: n2 - n1 + 1 }, (n, i) => i + n1).reduce((acc, curr) => acc + curr, 0);
}
...
function isTriangle(a, b, c) {
  ...
  for (let i = 0; i < args.length; i += 1) {
    const newArr = [...args];
    newArr.splice(i, 1);
    const x = newArr.reduce((n, m) => n + m, 0);
    if (x <= args[i]) {
      return false;
    }
  }
...
function getDigitalRoot(num) {
  ...
  while (arr.length !== 1) {
    arr = arr.map((x) => +x);
    arr = [...arr.reduce((a, b) => a + b, 0).toString()];
  }
  return +arr.join('');
}
...
function getCommonDirectoryPath(pathes) {
  ...
  for (let i = 0; i < pathesArr[0].length; i += 1) {
    const word = pathesArr[0][i];
    let counter = 0;
    for (let y = 1; y < pathesArr.length; y += 1) {
      if (pathesArr[y][i] === word) {
        counter += 1;
      }
    }
    if (counter === pathes.length - 1) {
      result.push(word);
    }
  }
  if (result.length > 0) {
